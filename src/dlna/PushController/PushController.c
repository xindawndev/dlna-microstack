...
char* _buildUri(struct AVRendererConnection* rendererConnection, PC_Token token, char* filename);
PC_Session PushController_PushFile(PC_Token token,
                                               struct AVRendererConnection* rendererConnection,
                                               char* filename, char* mimeType, char* protocolInfo,
...
char* _buildUri(struct AVRendererConnection* rendererConnection, ILibWebServer_ServerToken server, char* filename)
{
    int i;
    char tmp[1024];
    char* result = NULL;
    ...
    sprintf(tmp, "http://%s:%d/%s", rendererConnection->Parent->device->InterfaceToHost, ILibWebServer_GetPortNumber(server), escaped);
