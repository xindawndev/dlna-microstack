...
struct DMRCP_CP
{
   ...
   void (*RemoveSink)(struct UPnPDevice *device, int error_code);
...
struct DMRCP_CP * g_dmrcp_cp = NULL;
...
void DMRCP_CP_ProcessDeviceRemoval(struct DMRCP_CP* CP, struct UPnPDevice *device, int error_code);

static int CheckServiceAvaliable(struct UPnPService * svr)
{
    void * node;
    if (g_dmrcp_cp == NULL || ILibLinkedList_GetCount(g_dmrcp_cp->DeviceList) <= 0) return 0;
    node = ILibLinkedList_GetNode_Head(g_dmrcp_cp->DeviceList);
    while (node) {
        struct UPnPDevice * dev = (struct UPnPDevice *)ILibLinkedList_GetDataFromNode(node);
        struct UPnPService * sv = dev->Services;
        if (sv == NULL) return 0;
        while (sv) {
            if (svr == sv) return 1;
            sv = sv->Next;
   ...
   do
   {
      ...
      if(RetDevice!=NULL)
      {
         ...
         if(CP->DiscoverSink!=NULL)
         {
            ...
            ILibLinkedList_AddTail(CP->DeviceList, RetDevice);
         }
      }
   ...
   if(!(header==NULL || !ILibWebClientIsStatusOk(header->StatusCode)) && done!=0)
   {
      ...
      if(device->SCPDLeft==0)
      {
         ...
         else if(IsInterrupt==0)
         {
            ...
            // stale device
            //
            DMRCP_CP_ProcessDeviceRemoval(CP,device, -1);
...
void DMRCP_CP_RecursiveReleaseAndEventDevice(struct DMRCP_CP* CP, struct UPnPDevice *device)
{
   ...
   if(device->Reserved!=0)
   {
      ...
      {
         CP->RemoveSink(device, 0);
         if (ILibLinkedList_GetCount(CP->DeviceList) > 0)
             ILibLinkedList_Remove_ByData(CP->DeviceList, device);
      }
...
void DMRCP_CP_ProcessDeviceRemoval(struct DMRCP_CP* CP, struct UPnPDevice *device, int error_code)
{
   ...
   while(temp!=NULL)
   {
      DMRCP_CP_ProcessDeviceRemoval(CP, temp, error_code);
      temp = temp->Next;
   }
   ...
   if(device->Reserved!=0)
   {
      ...
      if(CP->RemoveSink!=NULL)
      {
         CP->RemoveSink(device, error_code);
         if (ILibLinkedList_GetCount(CP->DeviceList) > 0)
             ILibLinkedList_Remove_ByData(CP->DeviceList, device);
      }
      DMRCP_Release(device);
...
void DMRCP_SSDP_Sink(void *sender, char* UDN, int Alive, char* LocationURL, int Timeout,UPnPSSDP_MESSAGE m,void *cp)
{
   ...
   else
   {
      ...
      if(device!=NULL)
      {
         ...
         //
         ILibLifeTime_Remove(((struct DMRCP_CP*)cp)->LifeTimeMonitor,device);
         DMRCP_CP_ProcessDeviceRemoval(CP,device, 0);
         //
...
void DMRCP_StopCP(void *v_CP)
{
   ...
   ILibLinkedList_Destroy(CP->DeviceList);
...
*/
void *DMRCP_CreateControlPoint(void *Chain, void(*A)(struct UPnPDevice*),void(*R)(struct UPnPDevice*, int))
{
   ...
   cp->DeviceList = ILibLinkedList_Create();
   #ifdef UPNP_DEBUG
   ...
   cp->HTTP = ILibCreateWebClient(24,Chain);
...
static void DMRCP_ClearURI(struct DMRCP_CP * CP, char * UDN)
{
    struct UPnPDevice * dv = (struct UPnPDevice *)ILibGetEntry(CP->DeviceTable_UDN,UDN,(int)strlen(UDN));
    if(dv == NULL || ILibHasEntry(CP->DeviceTable_URI, dv->LocationURL, (int)strlen(dv->LocationURL)) == 0) {
        return;
    }

    ILibHashTree_Lock(CP->DeviceTable_URI);
    ILibDeleteEntry(CP->DeviceTable_URI, dv->LocationURL, (int)strlen(dv->LocationURL));
    ILibHashTree_UnLock(CP->DeviceTable_URI);
}

static void DMRCP_ClearAllURI(struct DMRCP_CP * CP)
{
    void * en = NULL;
    void * que = NULL;
    char * key = NULL;
    int keyLen = 0;
    void * data = NULL;
    struct UPnPDevice * dv = NULL;

    ILibHashTree_Lock(CP->DeviceTable_URI);

    que = ILibQueue_Create();
    en = ILibHashTree_GetEnumerator(CP->DeviceTable_URI);
    while (ILibHashTree_MoveNext(en) == 0) {
        ILibHashTree_GetValue(en, &key, &keyLen, &data);
        if(data != NULL) {
            ILibQueue_EnQueue(que, data);
        }
    }
    ILibHashTree_DestroyEnumerator(en);

    dv = (struct UPnPDevice *)ILibQueue_DeQueue(que);
    while (dv != NULL) {
        ILibDeleteEntry(CP->DeviceTable_URI, dv->LocationURL, (int)strlen(dv->LocationURL));
        dv = (struct UPnPDevice *)ILibQueue_DeQueue(que);
    }

    ILibQueue_Destroy(que);
    ILibHashTree_UnLock(CP->DeviceTable_URI);    
}

static void DMRCP_ClearUDN(struct DMRCP_CP * CP, char * UDN)
{
    if(ILibHasEntry(CP->DeviceTable_UDN,UDN,(int)strlen(UDN))==0) {
        return;
    }
...
static void DMRCP_ClearAllUDN(struct DMRCP_CP * CP)
{
    void * en = NULL;
    void * que = NULL;
    char * key = NULL;
    ...
    que = ILibQueue_Create();
    en = ILibHashTree_GetEnumerator(CP->DeviceTable_UDN);
    while (ILibHashTreeIsEmpty(CP->DeviceTable_UDN) == 0 && ILibHashTree_MoveNext(en) == 0) {
        ILibHashTree_GetValue(en, &key, &keyLen, &data);
        if(key != NULL) {
            key[keyLen] = '\0';
            ILibQueue_EnQueue(que, key);
        }
    }

    ILibHashTree_DestroyEnumerator(en);

    key = (char *)ILibQueue_DeQueue(que);
    while (key != NULL) {
        ILibDeleteEntry(CP->DeviceTable_UDN, key, (int)strlen(key));
        key = (char *)ILibQueue_DeQueue(que);
    }
    ILibQueue_Destroy(que);
    ILibHashTree_UnLock(CP->DeviceTable_UDN);
...
void DMRCP_SearchDevice(void * CPToken)
{
    ...
    ILibWebClient_ClearWCDO(CP->HTTP);
    //DMRCP_ClearAllURI(CP);
    DMRCP_ClearAllUDN(CP);

    ILibSSDPDeviceSearch(CP->SSDP);
}

void DMRCP_SearchDeviceWithUDN(void * CPToken, char * UDN)
{
    struct DMRCP_CP * CP = (struct DMRCP_CP *)CPToken;
    if (CP == NULL || CP->SSDP == NULL) return;
    ILibWebClient_ClearWCDO(CP->HTTP);
    DMRCP_ClearURI(CP, UDN);
    DMRCP_ClearUDN(CP, UDN);
    ILibSSDPDeviceSearch(CP->SSDP);
}
...
int *PAUSE)
{
   ...
   else
   {
      if(header==NULL)
      {
         /* Connection Failed */
          if (IsInterrupt != 6) {
              if (!CheckServiceAvaliable(Service)) {
              ...
              DMRCP_CP_ProcessDeviceRemoval(((struct DMRCP_CP *)Service->Parent->CP),Service->Parent, IsInterrupt);
              DMRCP_Release(Service->Parent);
