...
void DMR_ProcessUNSUBSCRIBE(struct packetheader *header, struct ILibWebServer_Session *session)
{
   ...
   lock_wait(&(((struct DMR_DataObject*)session->User)->EventLock));
   ...
   lock_post(&(((struct DMR_DataObject*)session->User)->EventLock));
}
...
int *PAUSE)
{
   ...
   {
      lock_wait(&(((struct DMR_DataObject*)upnp)->EventLock));
      ...
      lock_post(&(((struct DMR_DataObject*)upnp)->EventLock));
   }
}
void DMR_SendEvent_Body(void *upnptoken,char *body,int bodylength,struct SubscriberInfo *info)
{
...
void DMR_SendEvent(void *upnptoken, char* body, const int bodylength, const char* eventname)
{
   ...
   lock_wait(&(DMR_Object->EventLock));
   ...
   lock_post(&(DMR_Object->EventLock));
}
...
void DMR_StreamDescriptionDocument(struct ILibWebServer_Session *session, int intelnalerror)
{
   ...
   if (intelnalerror == 0)
   {
       ILibWebServer_StreamHeader_Raw(session,403,"PPTV DMR Internal Error",responseHeader,1);
       ILibWebServer_StreamBody(session,"Are you kidding me? laddy^_^",28,ILibAsyncSocket_MemoryOwnership_STATIC,1);
       return;
   }
