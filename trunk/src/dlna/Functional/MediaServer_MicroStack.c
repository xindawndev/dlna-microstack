...
void MediaServer_TryToSubscribe(char* ServiceName, long Timeout, char* URL, int URLLength,struct ILibWebServer_Session *session)
{
   ...
   if(*TotalSubscribers<10)
   {
      ...
      if(p->NumResults==1)
      {
          free(NewSubscriber);// maybe memory leak. add by leochen
          free(SID);// maybe memory leak. add by leochen
...

{
   ...
   if(MediaServer__Device_MediaServer_Impl.ModelURL == NULL) {MediaServer__Device_MediaServer_Impl.ModelURL = "http://www.pptv.com";}
