...
#ifndef _ILIBPARSERS_H_
#define _ILIBPARSERS_H_
...
#include <sys/timeb.h>
    ...
    #define lock_init(x, shared, initval) pthread_mutex_init(x, NULL)
    #define lock_destroy(x) pthread_mutex_destroy(x)
    #define lock_wait(x) pthread_mutex_lock(x)
    #define lock_trywait(x) pthread_mutex_trylock(x)
    #define lock_post(x) pthread_mutex_unlock(x)
#elif defined(_POSIX)
    #define lock_t sem_t
    #define lock_init(x, shared, initval) sem_init(x, shared, initval)
    #define lock_destroy(x) sem_destroy(x)
    #define lock_wait(x) sem_wait(x)
    #define lock_trywait(x) sem_trywait(x)
    #define lock_post(x) sem_post(x)
#endif
    ...
    #define lock_init(x,pShared,InitValue) *x=CreateSemaphore(NULL,InitValue,SEM_MAX_COUNT,NULL)
    #define lock_destroy(x) (CloseHandle(*x)==0?1:0)
    #define lock_wait(x) WaitForSingleObject(*x,INFINITE)
    #define lock_trywait(x) ((WaitForSingleObject(*x,0)==WAIT_OBJECT_0)?0:1)
    #define lock_post(x) ReleaseSemaphore(*x,1,NULL)
